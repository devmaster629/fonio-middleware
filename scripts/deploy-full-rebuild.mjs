...
// Full source deploy: whole src/, prisma/, public/ plus package manifests.
execSync(
  'tar -czf full-upload.tgz package.json package-lock.json src prisma public .env.example',
  { stdio: 'inherit' },
);
...
conn
  .on('ready', async () => {
    try {
      ...
      await upload(conn, ARCHIVE, '/tmp/full-upload.tgz');
      await exec(
        conn,
        `set -e
cd ${APP_DIR}
tar -xzf /tmp/full-upload.tgz -C ${APP_DIR}
rm -f /tmp/full-upload.tgz
ls src/admin src/common/guards
