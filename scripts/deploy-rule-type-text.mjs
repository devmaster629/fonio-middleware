#!/usr/bin/env node
/**
 * Deploy free-text approval rule types: schema migration + API + admin UI.
 * Usage: node scripts/deploy-rule-type-text.mjs
 */
...
const FILES = [
  ...
  'prisma/migrations/20260910120000_approval_rule_type_text/migration.sql',
  'src/admin/dto/admin-rules.dto.ts',
...
conn
  .on('ready', async () => {
    try {
      await exec(
        conn,
        `mkdir -p ${APP_DIR}/prisma/migrations/20260910120000_approval_rule_type_text`,
      );
